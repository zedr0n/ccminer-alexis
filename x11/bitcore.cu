...
extern "C" int scanhash_bitcore(int thr_id, struct work* work, uint32_t max_nonce, unsigned long *hashes_done)
{
	...
	//gpulog(LOG_INFO, thr_id, "Starting hashing: ");
	...
	do {
		...
		for (int i = 1; i < hashes; i++)
		{
			...
			case BLAKE:
				...
				quark_blake512_cpu_hash_64(thr_id, throughput, NULL, d_hash[thr_id]);
				TRACE("blake  :");
				break;
			case BMW:
		...
		*hashes_done = pdata[19] - first_nonce + throughput;
		//gpulog(LOG_INFO, thr_id, "Hashing: %d", hashes_done);
		...
		if (work->nonces[0] != UINT32_MAX)
		{
			...
			//gpulog(LOG_INFO, thr_id, "Successfully found nonce: %x", work->nonces[0] );
			...
			if (vhash[7] <= Htarg && fulltest(vhash, ptarget)) {
				...
				//gpulog(LOG_WARNING, thr_id, "Number of nonces found: ", res);
				return res;
